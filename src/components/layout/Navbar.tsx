...
const Navbar = () => {
  ...
  const NavLink = ({ to, label, isScrollLink = false }: { to: string; label: string; isScrollLink?: boolean }) => {
    ...
    return (
      <Link to={to} className="text-sm font-semibold text-slate-600 hover:text-teal-600 transition-colors">
        {label}
      </Link>
    );
  };
  ...
  const MobileNavLink = ({ to, label, isScrollLink = false }: { to: string; label: string; isScrollLink?: boolean }) => {
    if (isScrollLink && isHomePage) {
      return (
        <button
          onClick={() => scrollToSection(to.replace('#', ''))}
          className="text-sm font-bold text-slate-600 hover:text-teal-600 py-2 border-b border-slate-100 text-left w-full"
        >
          {label}
        </button>
      );
